...
export default function EditWarehouseComponent({match}) {
    const [warehouse, setWarehouse] = useState(null);
    const apiUrl = process.env.REACT_APP_API_URL;

    useEffect(() => {
        axios.get(`${apiUrl}/warehouses/${match.params.warehouseId}`)
            .then((response) => {
    ...
    const handleSubmit = (event) => {
        ...
        if(event.target.phone.value && !event.target.phone.value.match(validRegexPhone)) {
                return alert("Please enter valid phone number!")
        }

        if(event.target.email.value && !event.target.email.value.match(validRegexEmail)) {
    ...
    axios.put(apiUrl + "/warehouses/" + warehouseId, {
        id: warehouseId,
    ...
    return (
        <div className="main">
            <div className="edit-warehouse__header">
                <h2><NavLink to={'/warehouses/' + warehouse.id}><img className="edit-warehouse__back-icon" src={backImg} alt="" /></ NavLink> Edit Warehouse</h2>
            </div>
            <form className="edit-warehouse__form-container" onSubmit={handleSubmit} id='editWarehouse'>
                <div className="edit-warehouse__form edit-warehouse__form--left">
                    ...
                    <input className="edit-warehouse__input" name="name" id="name" placeholder={warehouse.name}></input>
                    <label className="edit-warehouse__label">Street Address</label>
                    <input className="edit-warehouse__input" name="address" id="address" placeholder={warehouse.address}></input>
                    <label className="edit-warehouse__label">City</label>
                    <input className="edit-warehouse__input" name="city" id="city" placeholder={warehouse.city}></input>
                    <label className="edit-warehouse__label">Country</label>
                    <input className="edit-warehouse__input" name="country" id="country" placeholder={warehouse.country}></input>
                </div>
                <div className="edit-warehouse__form">
                    ...
                    <input className="edit-warehouse__input" name="contactName" id="contactName" placeholder={warehouse.contact.name}></input>
                    <label className="edit-warehouse__label">Position</label>
                    <input className="edit-warehouse__input" name="position" id="position" placeholder={warehouse.contact.position}></input>
                    <label className="edit-warehouse__label">Phone Number</label>
                    <input className="edit-warehouse__input" name="phone" id="phone" placeholder={warehouse.contact.phone}></input>
                    <label className="edit-warehouse__label">Email</label>
                    <input className="edit-warehouse__input" name="email" id="email" placeholder={warehouse.contact.email}></input>
                </div>
            </form>
            <div className='edit-warehouse__button-container'>
                <NavLink className="edit-warehouse__button--navlink" to={'/warehouses/' + warehouse.id}>
